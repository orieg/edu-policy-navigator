...
// even if type declarations are missing for static import.
const kuzu = require('kuzu'); // Reverted this CJS import

const fs = require('fs'); // Changed to CJS import
const path = require('path'); // Changed to CJS import
// Removed: import { fileURLToPath } from 'url'; // Added for ES module __dirname equivalent

// Removed: ES module __dirname equivalent
// const __filename = fileURLToPath(import.meta.url);
// const __dirname = path.dirname(__filename); // __dirname is available in CommonJS
...
function generateCreateTableCypher(nodeDef /*: any*/) {
...
function generateCreateRelTableCypher(relDef /*: any*/) {
...
async function main() {
    ...
    if (fs.existsSync(DB_PATH)) {
        // Use rmSync for directories instead of unlinkSync
        fs.rmSync(DB_PATH, { recursive: true, force: true });
        console.log(`Deleted existing database directory: ${DB_PATH}`);
    }
    ...
    try {
        ...
        for (const cdsCode in districtData) {
            ...
            try {
                ...
                const preparedStmt = await conn.prepare(createCypher);
                await conn.execute(preparedStmt, params);
                districtCount++;
            } catch (err) {
        ...
        for (const districtCdsCode in schoolDataByDistrict) {
            ...
            for (const school of schools) {
                ...
                try {
                    // Prepare and execute school creation
                    const preparedSchoolStmt = await conn.prepare(createSchoolCypher);
                    await conn.execute(preparedSchoolStmt, schoolParams);
                    schoolCount++;

                    // Prepare and execute relationship creation
                    const preparedRelStmt = await conn.prepare(createRelCypher);
                    await conn.execute(preparedRelStmt, relParams);
                    relCount++;
                } catch (err) {
